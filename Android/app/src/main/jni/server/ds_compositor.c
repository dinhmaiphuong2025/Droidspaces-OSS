...
static void buffer_destroy_notify(struct wl_listener *listener, void *data) {
  ...
  if (!surf)
    return;
  if (surf->previous_buffer == buf)
    surf->previous_buffer = NULL;
  if (surf->current_buffer == buf)
    surf->current_buffer = NULL;
  if (surf->pending_buffer == buf)
...
static void arm_buffer(struct ds_surface *surf, struct ds_buffer *buf) {
  if (!buf || buf->is_shm || !buf->resource || buf->surface == surf)
    return;
  buf->surface = surf;
...
static void surface_commit(struct wl_client *client,
  ...
  if (surf->pending_buffer) {
    if (surf->current_buffer && surf->current_buffer != surf->pending_buffer) {
      if (surf->previous_buffer &&
          surf->previous_buffer != surf->current_buffer &&
          surf->previous_buffer != surf->pending_buffer) {
        if (surf->previous_buffer->resource) {
          wl_buffer_send_release(surf->previous_buffer->resource);
        }
        if (surf->previous_buffer->is_shm) {
          ds_shm_free_buffer(surf->previous_buffer);
        } else {
          disarm_buffer(surf->previous_buffer);
        }
      }
      surf->previous_buffer = surf->current_buffer;
    }
    ...
    /* Double buffering: release previous_buffer only after the new buffer
     * has been presented. The current_buffer remains owned by the compositor
     * and is never released early to avoid buffer contention / pointer flicker.
     */
    if (surf->previous_buffer &&
        surf->previous_buffer != surf->current_buffer) {
      if (surf->previous_buffer->resource) {
...
static void surface_offset(struct wl_client *client,
                           struct wl_resource *resource, int32_t sx,
                           int32_t sy) {
  (void)client;
  (void)resource;
  (void)sx;
...
static void surface_resource_destroy(struct wl_resource *resource) {
  ...
  if (surf->previous_buffer && surf->previous_buffer != surf->current_buffer) {
    if (surf->previous_buffer->is_shm) {
      ds_shm_free_buffer(surf->previous_buffer);
    } else {
      disarm_buffer(surf->previous_buffer);
...
static void compositor_create_surface(struct wl_client *client,
  ...
  wl_resource_set_implementation(surf->resource, &ds_surface_impl, surf,
...
int ds_compositor_init(struct ds_server *server) {
  server->compositor_global = wl_global_create(
      server->display, &wl_compositor_interface, 4, server, compositor_bind);
  server->subcompositor_global =
